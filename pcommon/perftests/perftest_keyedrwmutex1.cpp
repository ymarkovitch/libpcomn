...
/*******************************************************************************
 FILE         :   perftest_keyedrwmutex1.cpp
...
int main(int argc, char *argv[])
{
   ...
   pcomn::PTKeyedRWMutex<unsigned> kmi (4, 4) ;
   ...
   for (unsigned i = 0 ; i < count ; ++i)
      ...
      {
         kmi.lock_shared(slots1[n]) ;
         kmi.unlock(slots1[n]) ;
      }

   sw.stop() ;

   printf("\nCount=%u, %u key(s), %fs, %lu rlocks/s\n",
   ...
   printf("\nCount=%u, %u key(s), %fs, %lu wlocks/s\n",
          count, NKeys, sw.elapsed(), (unsigned long)(((double)count * NKeys)/sw)) ;
   return 0 ;
}
