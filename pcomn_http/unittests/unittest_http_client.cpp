...
*******************************************************************************/
class HttpClientTests : public CppUnit::TestFixture {
private:
  void Test_Simple_Stream() ;
    void Test_Socket_Bufstream() ;

    CPPUNIT_TEST_SUITE(HttpClientTests) ;
...
public:
    void setUp()
    {
        EchoServerName = CPPUNIT_AT_TESTDIR("testclient_svr.py ") ;
    }
} ;
...
void HttpClientTests::Test_Socket_Stream()
{
    ...
    SPAWN_ECHOSERVER(2) ;
    {
        net::stream_socket_ptr sock (new net::client_socket(net::sock_address(TEST_PORT))) ;
...
int main(int argc, char *argv[])
{
    ...
    runner.addTest(HttpClientTests::suite()) ;

    return
        pcomn::unit::run_tests(runner, argc, argv, "unittest.http.client.trace.ini",
                               "Testing HTTP client.") ;
}
